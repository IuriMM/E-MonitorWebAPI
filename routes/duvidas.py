...
async def update_duvida(id: str, duvida_update: DuvidaUpdate, current_user: dict = Depends(get_current_user)):
    ...
    # Security fix: Verify doubt exists and user is authorized to edit it
    duvida_atual = await db.duvidas.find_one({"_id": ObjectId(id)})
    if not duvida_atual:
        raise HTTPException(status_code=404, detail="Duvida not found")

    is_autor = duvida_atual.get("usuario") == str(current_user["_id"])
