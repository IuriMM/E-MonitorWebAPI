...
from models import MaterialEstudo, MaterialEstudoCreate, MaterialEstudoUpdate, Papel
...
async def create_material_estudo(material_estudo: MaterialEstudoCreate, current_user: dict = Depends(get_current_user)):
    ...
    material_dict["autor"] = current_user["nome"]
...
def _is_autor_ou_monitor(current_user: dict, material: dict) -> bool:
    if current_user["papel"] in (Papel.MONITOR, Papel.ADMIN):
        return True
    return material.get("autor") == current_user["nome"]
...
async def update_material_estudo(id: str, material_update: MaterialEstudoUpdate, current_user: dict = Depends(get_current_user)):
    ...
    if not material:
        raise HTTPException(status_code=404, detail="MaterialEstudo not found")
    if not _is_autor_ou_monitor(current_user, material):
        raise HTTPException(status_code=403, detail="Só o autor deste material ou monitor/admin pode editá-lo")

    update_data = {k: v for k, v in material_update.model_dump().items() if v is not None}
    if update_data:
        await db.materiais_estudo.update_one({"_id": ObjectId(id)}, {"$set": update_data})
...
async def delete_material_estudo(id: str, current_user: dict = Depends(get_current_user)):
    ...
    if not material:
        raise HTTPException(status_code=404, detail="MaterialEstudo not found")
    if not _is_autor_ou_monitor(current_user, material):
        raise HTTPException(status_code=403, detail="Só o autor deste material ou monitor/admin pode excluí-lo")
    await db.materiais_estudo.delete_one({"_id": ObjectId(id)})
